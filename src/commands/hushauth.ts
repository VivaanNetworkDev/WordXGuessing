...
composer.command("Wordauth", async (ctx) => {
  ...
  if (action === "list") {
    ...
    return await ctx.reply(
      `<b>🔐 Authorized Users for Word Game</b>\n\n${userList}`,
      replyConfig,
    );
  }
...
CommandsHelper.addNewCommand(
  "Wordauth",
  "Manage users authorized to end the Word game (admin only)",
);

export const WordAuthCommand = composer;
